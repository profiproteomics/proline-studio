...
public class FeaturesPanel extends AbstractPeakelsPanel {
  ...
  public FeaturesPanel(IPeakelViewer peakelViewer) {
    super(peakelViewer);
  ...
  private void matchFeatures(List<Double> mzValues, List<Double> rtValues, List<Integer> zValues) {
    ...
    List<IFeature> matched = new ArrayList<>();
    ...
    for (int k = 0; k < mzValues.size(); k++) {
      ...
      if (features != null) {
        ...
        if (feature != null) {
          matchingCount++;
          matched.add(feature);
          logger.info("Feature matching {}, {}, {}+ = {}, {}, {}+  ---> {}", mz, rt, z, feature.getMz(), feature.getElutionTime(), feature.getCharge(), feature.getPeakelsCount());
        ...
        BaseFeature f = new BaseFeature(mz, (float) rt, (float) rt, (float) rt, null, 1);
        f.setCharge(z);
        notFound.add(new BaseFeature(mz, (float) rt, (float) rt, (float) rt, null, 1));
        logger.warn("no feature found for {}, {}, {}+", mz, rt / 60.0, z);
      }
    }
    ...
    matched.forEach(feature -> logger.info("matched Feature  = {}, {}, {}, {} ", feature.getMz(), feature.getElutionTime() / 60.0, feature.getCharge(), feature.getPeakelsCount()));
