 ...
 * @author CB205360
 */
public class SignalViewerBuilder {
    
     static Container buildEditor(List<Signal> signals) {
         return SignalViewerBuilder.buildEditor(signals, true);         
     }
    
     static Container buildEditor(List<Signal> signals, boolean editable) {
        if (signals.size() == 1) {
            if(editable)
                return new SignalEditorPanel(signals.get(0));
            else
                return new SignalPanel(signals.get(0));
        } else {
