...
/**
 * Task to export a dataset (it could be identification, XIC or SC...)
 * It uses a configuration file to specify the export.
 * It will replace ExportXICTask and ExportRSMTask
 * @author MB243701
 */
public class ExportDatasetTask extends AbstractServiceTask {
    ...
    public ExportDatasetTask(AbstractServiceCallback callback, DDataset dataset, HashMap<String,Object> exportParams, boolean prideFormat, List<String> filePathInfo) {
        ...
        if(m_export2Pride)
            ...
            super.setTaskInfo( new TaskInfo("Export Dataset " + dataset.getName(), true, TASK_LIST_INFO, TaskInfo.INFO_IMPORTANCE_HIGH));
    }
    ...
    public boolean askService() {
        ...
        rsmIdent.put("ds_id", m_dataset.getId());
        Long rsmId = m_dataset.getResultSummaryId();
        if (m_dataset.getMasterQuantitationChannels() != null && !m_dataset.getMasterQuantitationChannels().isEmpty()){
            rsmId = m_dataset.getMasterQuantitationChannels().get(0).getQuantResultSummaryId();
        }
