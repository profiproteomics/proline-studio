...
public abstract class Axis {
   ...
   public static class AxisRangePanel extends JPanel {
      ...
      private void updateRange() {
         ...
         if (!m_maxTextField.getText().isEmpty()) {
            try {
               double max = Double.parseDouble(m_maxTextField.getText().trim());
               m_axis.setRange(m_axis.getMinValue(), max);
            } catch (NumberFormatException nfe) {  }

         }
         
         if (!m_minTextField.getText().isEmpty()) {
            try {
               double min = Double.parseDouble(m_minTextField.getText().trim());
               m_axis.setRange(min, m_axis.getMaxValue());
            } catch (NumberFormatException nfe) {  }
         }
