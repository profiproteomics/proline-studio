...
/**
 * Panel with a table used to compare two peakel lists which should be very similar. A status shows the potential errors.
 */
public class PeakelsComparePanel  extends AbstractPeakelsPanel  {
    ...
    protected List<IPeakel> getSelectedIPeakels() {
        if ((m_pairedPeakelArrayList != null) && (!m_pairedPeakelArrayList.isEmpty()) && (m_table.getSelectedRowCount() > 0)) {
            ...
            for (int row : selectedRows) {
                ...
                IPeakel p2 = pairedPeakel.getPeakel2();
                if (p2 != null) {
    ...
    protected void matchIons(List<BaseFeature> ions, ImportedDataTableModel importedTableModel) {
        ...
        IRawFile rawFile = m_pairedPeakelArrayList.get(0).getPeakel1().getRawFile();  //JPM.TODO
        int matchingCount = 0;
...
//    }
    ...
    private PeakelsHelper getPeakelsHelper() {
        /*if (m_helper == null) {
            Peakel[] apeakels = m_peakels1.stream().map(p -> p.getPeakel()).toArray(s -> new Peakel[s]);
    ...
    public void setPeakels(ArrayList<PairedPeakel> pairedPeakelArrayList) {
        ...
        m_helper = null;
        ((PeakelsCompareTableModel)m_compoundTableModel.getBaseModel()).setPeakels(pairedPeakelArrayList);
